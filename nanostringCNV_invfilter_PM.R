## Script to process Nanostring CNV data without considering the 12 invariant probes that are "variant"
...
remove_inv_probes <- read.csv(file=paste0(prefix, "/nanostringCNV_variantprobes.txt"), header = T, sep = "\t")
#randINVmeancount <- mean(sapply(list.files(path = "/Users/aanu/Desktop/AWS/storage/patients/cnv", pattern = "nanostringCNV.*\\.RCC$", recursive = TRUE, full.names = TRUE), function(i) mean(subset(readRaw(i)$inv, !(readRaw(i)$inv$Name %in% remove_inv_probes$invar_probes))$Count)))
randINVmeancount <- 1400.333
...
normNS <- function(NSdat, NSfile) {
  ...
  invnf <- randINVmeancount / mean(subset(NSdat$inv, !(NSdat$inv$Name %in% remove_inv_probes$invar_probes))$Count)
  #subset(NSdat$inv, !(NSdat$inv$Name %in% remove_inv_probes$invar_probes))$Count
...
write.table(final.df, file=paste0(prefix, "/", sample, "/", sample, ".nanostringCNV.invfilter.out"), sep="\t", row.names=FALSE, quote=FALSE)
